...
class ESP32DroneTracker:
 ...
 print("SUCCESS: ESP32 connection successful!")
 self.start_sweep_mode()
 else:
 print("ERROR: ESP32 connection failed - check IP and network")
 ...
 print(f"ERROR: ESP32 HTTP error: {response.status_code}")
 if attempt < max_retries - 1:
 time.sleep(0.1) # Brief delay before retry
 continue

 except requests.exceptions.Timeout:
 print(f"WARNING: ESP32 timeout (attempt {attempt + 1}/{max_retries})")
 if attempt < max_retries - 1:
 time.sleep(0.05) # Brief delay before retry
 continue
 except requests.exceptions.ConnectionError:
 print(f"WARNING: ESP32 connection lost (attempt {attempt + 1}/{max_retries})")
 if attempt < max_retries - 1:
 time.sleep(0.1) # Brief delay before retry
 continue
 except Exception as e:
 print(f"WARNING: ESP32 error: {str(e)[:30]}")
 break
 ...
 print(f"WARNING: Center timeout (attempt {attempt + 1}/{max_retries})")
 if attempt < max_retries - 1:
 time.sleep(0.1)
 continue
 except Exception as e:
 print(f"WARNING: Center error: {str(e)[:30]}")
 break
 return False
 ...
 print(f"WARNING: Failed to start ESP32 sweep: {e}")
 return False
 ...
 if response.status_code == 200:
 print("STOP: ESP32 movement stopped")
 self.tracking_active = False
 return True
 except Exception as e:
 print(f"WARNING: Failed to stop ESP32 movement: {e}")
 return False
 ...
 print(f"WARNING: Failed to get ESP32 status: {e}")
 return None
