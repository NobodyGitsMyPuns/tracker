...
def create_info_panel(info_data):
    ...
    # AI Parallax Correction Info
    if 'ai_debug' in info_data and info_data['ai_debug']:
        ai_data = info_data['ai_debug']
        cv2.putText(panel, "AI PARALLAX CORRECTION:", (10, y_pos), 
                   cv2.FONT_HERSHEY_SIMPLEX, config.FONT_SCALE_MEDIUM, config.COLOR_YELLOW, 2)
        y_pos += line_height
        
        status = ai_data.get('status', 'unknown')
        distance = ai_data.get('estimated_distance_m', 0)
        cv2.putText(panel, f"STATUS: {status.upper()}", 
                   (20, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        y_pos += line_height
        
        if distance > 0:
            cv2.putText(panel, f"DISTANCE: {distance:.1f}m", 
                       (20, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            y_pos += line_height
        
        if 'correction_offset' in ai_data:
            offset_x, offset_y = ai_data['correction_offset']
            cv2.putText(panel, f"CORRECTION: X={offset_x:+.0f}px, Y={offset_y:+.0f}px", 
                       (20, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            y_pos += 35
...
def main():
    ...
    try:
        ...
        while True:
            try:
                choice = input(f"\nSelect model (1-{len(models)}) [default: 4 for YOLOv8x]: ").strip()
                if choice == "":
                    choice = "4"  # Default to YOLOv8x
                choice = int(choice)
    ...
    try:
        ...
        print(f"\n⚠️  IMPORTANT: YOLO was trained on 80 common household objects:")
        print("✅ Good at: people, furniture, vehicles, animals, common electronics")
        print("❌ Bad at: Arduino boards, chips, cables, specific tech gear, lab equipment")
    ...
    if esp32_tracker.test_connection():
        ...
        add_log("❌ ESP32 connection FAILED - check network")
    ...
    while gui_running:
        ...
        if frame_count % config.DEBUG_FRAME_INTERVAL == 0:
            ...
            print(f"CROSSHAIRS - Center: ({static_center_x}, {static_center_y}) | AI: ({ai_crosshair_x}, {ai_crosshair_y}) | Status: {status} | Distance: {distance:.1f}m")
        ...
        # Only calculate offsets and servo movements if drone was detected
        if drone_center_x is not None and drone_center_y is not None:
            ...
            # Store drone position data for info panel
            drone_position_data = {
                'x_dir': x_dir,
                'y_dir': y_dir,
            ...
            # No drone detected
            drone_position_data = None
        
        # Prepare data for info panel (including AI debug info)
        info_data = {
        ...
        ai_text = f" | 🎯 AI TARGET" if ai_status == 'ai_corrected' else ""
        status_text = f"🎮 MANUAL CONTROL | WASD=2° | Arrow Keys=15° | TAB=Auto-align{ai_text} | SPACE=🔥FIRE | C=center{zoom_text} | H=help | Q=quit"
        ...
        # DRAW CROSSHAIRS LAST (so they're always on top!)
        # 1. STATIC CENTER CROSSHAIR (ALWAYS VISIBLE)
        cv2.line(annotated_frame, (static_center_x - 30, static_center_y), (static_center_x + 30, static_center_y), green, 4)
        cv2.line(annotated_frame, (static_center_x, static_center_y - 30), (static_center_x, static_center_y + 30), green, 4)
        cv2.circle(annotated_frame, (static_center_x, static_center_y), 8, white, -1)  # White center dot
        cv2.circle(annotated_frame, (static_center_x, static_center_y), 8, green, 3)   # Green border
        cv2.putText(annotated_frame, "CENTER", (static_center_x - 35, static_center_y - 40), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, green, 2)
        
        # 2. AI PREDICTIVE CROSSHAIR (only when targeting)
        ai_status = ai_debug.get('status', 'unknown')
        if ai_status == 'ai_corrected':
            # AI has a target - draw predictive crosshair
            confidence = ai_debug.get('confidence', 0)
            
            # Dynamic crosshair based on confidence
            line_thickness = 4 if confidence > 0.7 else 3
            crosshair_size = 35 if confidence > 0.7 else 30
            
            # Red crosshair for AI prediction
            cv2.line(annotated_frame, (ai_crosshair_x - crosshair_size, ai_crosshair_y), 
                    (ai_crosshair_x + crosshair_size, ai_crosshair_y), red, line_thickness)
            cv2.line(annotated_frame, (ai_crosshair_x, ai_crosshair_y - crosshair_size), 
                    (ai_crosshair_x, ai_crosshair_y + crosshair_size), red, line_thickness)
            
            # Distinctive AI crosshair center (diamond shape)
            points = np.array([[ai_crosshair_x, ai_crosshair_y - 12], 
                              [ai_crosshair_x + 12, ai_crosshair_y], 
                              [ai_crosshair_x, ai_crosshair_y + 12], 
                              [ai_crosshair_x - 12, ai_crosshair_y]], np.int32)
            cv2.fillPoly(annotated_frame, [points], white)
            cv2.polylines(annotated_frame, [points], True, red, 2)
            
            # AI target info
            target_class = ai_debug.get('target_class', 'unknown')
            distance_text = ""
            if 'estimated_distance_m' in ai_debug:
                distance_text = f" ({ai_debug['estimated_distance_m']:.1f}m)"
            
            cv2.putText(annotated_frame, f"AI TARGET: {target_class.upper()}{distance_text}", 
                       (ai_crosshair_x - 70, ai_crosshair_y - 55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, red, 2)
            
            # Draw line connecting center to AI prediction (shows correction)
            cv2.line(annotated_frame, (static_center_x, static_center_y), 
                    (ai_crosshair_x, ai_crosshair_y), yellow, 2)
        ...
        if len(results[0].boxes) > 0:
            print(f"\n🔥🔥�� FRAME {frame_count} - FOUND {len(results[0].boxes)} OBJECTS! 🔥🔥🔥")
            ...
            for det in all_detections:
                ...
                if final_detections:
                    ...
                    print("🔍 SCANNING... No confident detections (try moving drone closer or into better lighting)")
        else:
        ...
        if key is not None:
            ...
            if esp32_tracker:
                    ...
                    if key == 82 or key == ord('w'):  # Up arrow or W - Tilt up
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                add_log(f"✅ {response.text}")
                            else:
                                add_log(f"❌ Tilt UP - HTTP {response.status_code}")
                        except Exception as e:
                            add_log(f"❌ Tilt UP - {str(e)[:30]}")
                        moved = True
                        
                    elif key == 84 or key == ord('s'):  # Down arrow or S - Tilt down  
                        step_text = f"{current_step}° ({key_type})"
                        add_log(f"⬇️ Tilt DOWN {step_text}")
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                add_log(f"❌ Tilt DOWN - HTTP {response.status_code}")
                        except Exception as e:
                            add_log(f"❌ Tilt DOWN - {str(e)[:30]}")
                        moved = True
                        
                    elif key == 81 or key == ord('a'):  # Left arrow or A - Pan left
                        step_text = f"{current_step}° ({key_type})"
                        add_log(f"⬅️ Pan LEFT {step_text}")
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                add_log(f"❌ Pan LEFT - HTTP {response.status_code}")
                        except Exception as e:
                            add_log(f"❌ Pan LEFT - {str(e)[:30]}")
                        moved = True
                        
                    elif key == 83 or key == ord('d'):  # Right arrow or D - Pan right
                        step_text = f"{current_step}° ({key_type})"
                        add_log(f"➡️ Pan RIGHT {step_text}")
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                add_log(f"❌ Pan RIGHT - HTTP {response.status_code}")
                        except Exception as e:
                            add_log(f"❌ Pan RIGHT - {str(e)[:30]}")
                        moved = True
                except Exception as e:
                    add_log(f"❌ Movement ERROR: {str(e)[:50]}")
                ...
                if key == ord('c'):  # 'C' key - Center servos
                    if esp32_tracker:
                        try:
                            ...
                            if success:
                                add_log("✅ Servos centered - SUCCESS")
                            else:
                                add_log("❌ Center servos - FAILED")
                        except Exception as e:
                            add_log(f"❌ Center ERROR: {str(e)[:50]}")
                        
                if key == ord('x'):  # 'X' key - Stop all movement
                    if esp32_tracker:
                        try:
                            add_log("⏸️ Stopping movement...")
                            success = esp32_tracker.stop_movement()
                            if success:
                                add_log("✅ Movement stopped - SUCCESS")
                            else:
                                add_log("❌ Stop movement - FAILED")
                        except Exception as e:
                            add_log(f"❌ Stop ERROR: {str(e)[:50]}")
                            
                if key == 32 or key == ord(' '):  # Spacebar - Fire blaster! (try both codes)
                    add_log(f"🔍 SPACEBAR detected! Key code: {key}")
                    if esp32_tracker:
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                add_log("🔥 BLASTER FIRED! (150ms)")
                            else:
                                add_log(f"❌ Fire FAILED - HTTP {response.status_code}")
                        except Exception as e:
                            add_log(f"❌ Fire FAILED - {str(e)[:30]}")
                    else:
                        add_log("❌ No ESP32 tracker available")
                ...
                if key == ord('+') or key == ord('='):  # '+' key - Zoom in
                ...
                elif key == ord('b'):  # 'B' key - Toggle drone-only mode
                    ai_parallax_correction.ai_parallax.drone_only_mode = not ai_parallax_correction.ai_parallax.drone_only_mode
                    mode_status = "ON" if ai_parallax_correction.ai_parallax.drone_only_mode else "OFF"
                    add_log(f"DRONE-ONLY MODE: {mode_status}")
                        
                elif key == ord('h'):  # 'H' key - Show help
                    print("\n" + "="*50)
                    print("🎮 MANUAL CAMERA CONTROL:")
                    print("="*50)
                    print("🕹️ MOVEMENT:")
                    ...
                    print("   B - Toggle drone-only mode (reduces flicker)")
                    print("   SPACEBAR - FIRE BLASTER! (150ms)")
                    print("   H - Show this help")
                    print("   Q - Quit")
                    print("="*50)
                    print("🚁 Drones will be detected and highlighted")
                    print("🎮 Use manual controls to aim at targets")
                    print("="*50)
